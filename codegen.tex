...
There was an idea to create child package to encapsulate one feature group in it. However, SPARK Ada does not allow to access child packages private part from parent. That will require to expose feature group internal variable, which will have to be accessible globally. It is definitely not good solution. Thus, feature group is translated with prefix \lstinline{Feature_Group_Name_*}. Feature group mapping is presented in section \ref{codegen:mapping:packages}, in listings \ref{lst:aadl_sample} and \ref{lst:package_mapping}.
...
The essence of AADL models is port communication. AADL ports can be declared in subprograms, threads, processes, systems and other entities. 


System consists of process(es), process consists of threads. For sample ports mapping, ports are exposed using \lstinline{system}. Communication between two systems has to be described by another system. Figure \ref{figure:port_communication} presents communication between two systems: panel and pump. AADL model of this system comprises 3 packages: \lstinline{Main}, \lstinline{Panel} and \lstinline{Pump}. They are presented on listing \ref{listing:port_communication}. \lstinline{Panel} package has on thread \lstinline{Panel_Thread} with two \lstinline{out} ports: \lstinline{event port} and \lstinline{event data port}. Both ports are exposed by process \lstinline{panel_process} and then by system \lstinline{panel}. \lstinline{Pump} package has similar structure, but two \lstinline{in} ports. Both are also exposed by process (\lstinline{pump_process}) and system (\lstinline{pump}). Connection between these two packages are defined in \lstinline{Main} package.
...
\singlespacing
\begin{lstlisting}[language=aadl, frame=single, gobble=0, caption={AADL model of port communication}, label={listing:port_communication}]
	package Panel
	public
	with Base_Types;
...
Based on mappings from section \ref{codegen:mapping}, conforming SPARK Ada code is presented on listing \ref{listing:port_communication_spark}. There are two packages: \lstinline{Panel} and \lstinline{Pump}. \lstinline{Main} package is omitted. Both contain procedures represented ports, according to ports mapping from section \ref{codegen:mapping:ports}. Additionally, both consists of empty thread declarations and bodies, which conforms to translations from section \ref{codegen:mapping:threads}.
...
In given example, communication is one way: from \lstinline{Panel} package to \lstinline{Pump} package. Thus, \lstinline{Pump} package does not need to know that \lstinline{Panel} package exists. In other words: it does not need to "with" it. However, if two way communication is needed (from Pump to Panel), then \lstinline{Pump} package has to "with" \lstinline{Panel} package. Modified model of previous example is depicted in the figure \ref{figure:port_communication_two_way} and presented on listing \ref{listing:port_communication_two_way}. Now, flow rate is sent by pump to panel.

\begin{figure}[ht]%t=top, b=bottom, h=here
    \begin{center}
    	\includegraphics[height=2.5in]{figures/port-communication-two-way.png}
    	\caption{Example of two way port communication}
    \end{center}
    \label{figure:port_communication_two_way}
\end{figure}

\singlespacing
\begin{lstlisting}[language=aadl, frame=single, gobble=0, caption={AADL model of two way port communication}, label={listing:port_communication_two_way}]
	package Panel
	public
	with Base_Types;
		...
		process implementation panel_process.imp
			...
			connections
				sbp: port panel_thread.Start_Button_Pressed->Start_Button_Pressed;
				fr: port Flow_Rate->panel_thread.Flow_Rate;
		end panel_process.imp;
		...
		system implementation panel.imp
			...
			connections
				sbp: port panel_process.Start_Button_Pressed->Start_Button_Pressed;
				fr: port Flow_Rate->panel_process.Flow_Rate;
		end panel.imp;
	...
	with Base_Types;
		...
		process implementation pump_process.imp
			...
			connections		
				sbp: port Start_Button_Pressed->Rate_Controller.Start_Button_Pressed;
				fr: port Rate_Controller.Flow_Rate->Flow_Rate;
		end pump_process.imp;
		...
		system implementation pump.imp
			...
			connections
				sbp: port Start_Button_Pressed->pump_process.Start_Button_Pressed;
				fr: port pump_process.Flow_Rate->Flow_Rate;
		end pump.imp;
	end Pump;
...
This model, translated to SPARK Ada is presented in listing \ref{listing:port_communication_two_way_spark}. It will compile without any problems, but verification with SPARK Examiner will return following error: \lstinline{Semantic Error 135 - The package Panel is undeclared or not visible, or there is a circularity in the list of inherited packages}. This is valid Ada program, but not valid SPARK program. Now, the question is if two way communication between components should be allowed? Maybe SPARK Examiner is right forbidding such solution. 

\singlespacing
\begin{lstlisting}[language=ada, frame=single, gobble=0, caption={Two way port communication translated to SPARK Ada}, label={listing:port_communication_two_way_spark}]
	with Pump;
	with Base_Types;
	--# inherit Pump,
	--#         Base_Types;
...
One workaround is to use \lstinline{--# hide} annotation in body of procedure \lstinline{Send_Flow_Rate} and "with" \lstinline{Panel} package in body file instead of spec file. Modified \lstinline{Pump} package is presented in listing \ref{listing:port_communication_two_way_pump_package_fix}. Now, Examiner returns only warning, that subprogram \lstinline{Send_Flow_Rate} will be ignored: \lstinline{Warning  10 - The body of subprogram Send_Flow_Rate is hidden - hidden text is ignored by the Examiner}. However, it is still not good solution. It is one of the hardest translation issues encountered in this thesis. From AADL perspective, two-way communication is legal. On the other hand it is not legal in SPARK Ada. Two-way port communication requires further investigation, which is omitted in this thesis.


\singlespacing
\begin{lstlisting}[language=ada, frame=single, gobble=0, caption={Modified \lstinline{Pump} package, which passes Examiner syntax check}, label={listing:port_communication_two_way_pump_package_fix}]
	with Base_Types;
	--# inherit Base_Types;
	package Pump
	--# own task rc : Rate_Controller;
...
The ultimate goal is to create translator, which performs translations described in \ref{codegen:mapping} automatically. Automatic translator will enable translation of entire model and parts of the model. Initially, following functions will be supported:
\begin{itemize}
...
Second step, would be introduce BLESS support. Which means, add supported BLESS constructs described in section \ref{codegen:mapping:bless}:
\begin{itemize}
	\item assertions for threads
	\item pre- and postconditions for subprograms
\end{itemize}

Translator will be created in Scala [Sam is using Java?] programming language. During translator development Unit tests and Test Driven Development would be useful. Translation schemes can be used as input and expected output of particular test cases. Development should be performed incrementally. From adding translation for the simples constructions, like data types or single ports, to supporting entire packages mapping and BLESS.
