...
To create a task, the task type has to be declared and task variable of this type. Tasks can be declared only in packages. Not in subprograms or in other tasks. \cite{Barnes:Book} The priority of each tasks has to be specified by \lstinline{pragma Priority}.  Listing \ref{lst:SampleTask} shows sample package with two tasks.
...
Separated sequential parts can be verified (Odometer?). Sequential version of Max_Drug_Per_Hour_Watcher?
